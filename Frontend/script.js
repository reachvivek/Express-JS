	...
	function displayData(res=[]){
		if (res.data.length>0){
			res.data.map(bookings=>{
				...
				listItem.innerHTML=(`${bookings.name} <span>${bookings.phone}</span> ${bookings.email} <button id=${bookings.id} class='edit-btn'>Edit</button><button id=${bookings.id} class='del-btn'>x</button>`)
